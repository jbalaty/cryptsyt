module.exports = function (grunt) {
  var express = require('express'),
      ...
      fs = require('fs'),
      path = require('path'),
      custom_mime = require('express-custom-mime-types'),
...
//    var cm = function (req, res, next) {
//      var mimeType;
//      mimeType = path.extname(req.path);
//      if (typeof mimeType === 'string' && mimeType === '.webapp') {
...
//      app.use(static({ file: '/manifest.webapp', mime:'application/x-web-app-manifest+json' }));
  ...
  function static(options) {
    return function (req, res, next) { // Gotta catch 'em all (and serve index.html)
      ...
      fs.stat(filePath, function (err, stats) {
        ...
        // set mime type
        if (options.mime) {
          res.header('Content-Type', options.mime);
        }
        // Serve the file
